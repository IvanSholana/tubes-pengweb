...
const FacilityForm = ({ DataFacility }) => {
    ...
    const [formFacilityUpdate, setFormDataUpdate] = useState({
        judul: "",
        deskripsi: "",
        foto: null,
    });
    ...
    const handleFormUpdate = () => {
        // Mengirim data ke rute Laravel menggunakan inertia.post()
        Inertia.post(`/facility/update/${id}`, formFacilityUpdate, {
            onSuccess: () => {
                // Redirect ke halaman lain jika diperlukan
                Inertia.visit("/facility");
            },
    ...
    const handleDeleteData = () => {
        Inertia.post(`/facility/delete/${id}`, {
            onSuccess: () => {
                Inertia.visit("/facility");
            },
            onError: (errors) => {
                            ...
                            <div className="card-actions justify-end">
                                <div className="space-x-2">
                                    <Dialog.Root>
                                        <Dialog.Trigger asChild>
                                            <button
                                                className=" bg-slate-800 text-white py-2 px-5 mt-2 -mb-2 rounded-md"
                                                onClick={() => {
                                                    updateId(data.id);
                                                }}
                                            >
                                        ...
                                        <Dialog.Portal>
                                            ...
                                            >
                                                ...
                                                <Dialog.Description className="text-mauve11 mt-[10px] mb-5 text-[15px] leading-normal">
                                                    Isikan judul dan deskripsi
                                                    dari fasilitas baru yang
                                                    akan ditambahkan
                                                </Dialog.Description>
                                                <form
                                                    method="POST"
